...
exports.insertValues = function(values, modelName, transaction = null) {
	...
	if (transaction === null) {
		...
		return new Promise((resolve, reject) => {
			Model
				.create(values, {transaction: transaction})
				.then((result) => resolve({result: result._options.isNewRecord, record: result.dataValues}))
				.catch((err) => reject(err));
		});
	}
...
exports.updateValues = function(values, anyWhere, modelName, transaction = null) {
	const Model = require('../models/'+ modelName);
	if (transaction === null) {
		return new Promise((resolve, reject) => {
			Model
		...
		return new Promise((resolve, reject) => {
			Model
				.update(values, { where: anyWhere, transaction: transaction })
				.then((result) => resolve({result: result[0] == 1 ? true : false}))
				.catch((err) => reject(err));
		});
	}
